...
```
#change electricity used to electricity produced 
...
```{r}
grid.arrange(solar,wood,geo,wind, ncol=2,nrow=2, top=textGrob(expression(bold(underline("Renewable Energy Sources"))),gp=gpar(fontsize=20,font=3)))
...
```
After the adjustments only the average windspeed and the average sun hours appear to be factors for the amount of renewables that are being used in a state. So only the natural factors, that decide over the productiveness of the 
